 ...
 * can draw_line to the screen using the SDL rendering APIs. Without DRM master, SDL
 * is still able to process input and query attributes of attached displays,
 * but it cannot change display state or draw_line to the screen directly.
 *
 ...
 *
 *  Up to SDL 2.0.9, the render API would draw_line immediately when requested. Now
 *  it batches up draw_line requests and sends them all to the GPU only when forced
 ...
 *  registers_t one of these callback, the OS will consider the event
 *  un-handled, and it will apply its default back button behavior (terminate
 *  the app).
 *
 *  SDL registers_t its own back-button-press callback with the Windows Phone
