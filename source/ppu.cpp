...
void Ppu::oamfetch() {
    if (ticks == 40) {
        ...
        tileMapRowAddr = (bus->ppu_registers->lcdc.window_tile_map_area ? 0x9C00 : 0x9800) +
                         uint8_t((bus->ppu_registers->ly - bus->read_v(0xFF4A)) / 8) * 32;
...
uint8_t Ppu::oamtransfer(bool transparant_bg) {
    oam_t oam;
    uint8_t sprite_pixel = 0;
    for (int i = 0; i < 40; i++) {
        oam = oams[i];
        if ((uint8_t) (x - oam.position_x) < 8 && bus->ppu_registers->lcdc.obj_enable && oams_selection[i]) {
            uint16_t offset;
            offset = 0x8000;
...
void Ppu::pixeltransfer() {
    ...
    if (!fetcher->fifo_bg.empty()) {
        ...
        if (bus->ppu_registers->lcdc.window_enable) {
            window_fetcher->tick();
            if (wy <= bus->ppu_registers->ly && x >= 7 && x >= wx) {
                if (!window_fetcher->fifo_bg.empty()) {
                    window_pixel = window_fetcher->fifo_bg.front();
                    window_fetcher->fifo_bg.pop();
                    if (draw_window(bus)) lcd->write_pixel(x - 7, bus->ppu_registers->ly, window_pixel);
                }
            }
        }
        oamtransfer(!pixel && (!window_pixel || !bus->ppu_registers->lcdc.window_enable));
        //if(!pixel && !window_pixel) lcd->write_pixel(x, bus->ppu_registers->ly, oamtransfer(), 1, true);
        x++;
    }
}
